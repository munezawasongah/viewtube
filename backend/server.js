...
// Regional mobile-money number validation for tips.
// Returns { e164, local } or null. `local` is the 0-prefixed form users dial.
const TIP_COUNTRIES = {
  KE: { code: '254', label: 'Kenya',    currency: 'KES', networks: ['M-Pesa', 'Airtel Money'] },
  UG: { code: '256', label: 'Uganda',   currency: 'UGX', networks: ['MTN MoMo', 'Airtel Money'] },
  TZ: { code: '255', label: 'Tanzania', currency: 'TZS', networks: ['M-Pesa', 'Tigo Pesa', 'Airtel Money', 'Halopesa'] },
  RW: { code: '250', label: 'Rwanda',   currency: 'RWF', networks: ['MTN MoMo', 'Airtel Money'] },
};

function normalizeRegionalPhone(input, country) {
  const c = TIP_COUNTRIES[country];
  if (!c) return null;
  let d = String(input || '').replace(/\D/g, '');
  if (d.startsWith('0')) d = c.code + d.slice(1);
  else if (!d.startsWith(c.code)) d = c.code + d;
  // National number after country code should be 9 digits across these four countries
  if (!new RegExp('^' + c.code + '\\d{9}$').test(d)) return null;
  const local = '0' + d.slice(c.code.length);
  return { e164: d, local };
}
...
app.get('/videos/:id', optionalAuth, wrap(async (req, res) => {
  ...
  const cd = channelDoc.exists ? channelDoc.data() : null;
  const channelTip = (cd && cd.tipEnabled && cd.tipNumberLocal && cd.tipCountry) ? {
    enabled: true, number: cd.tipNumberLocal, network: cd.tipNetwork || null,
    country: cd.tipCountry, currency: (TIP_COUNTRIES[cd.tipCountry] || {}).currency || null,
  } : { enabled: false };
  res.json({ id: doc.id, ...v, liked, subscribed, myReaction, channel: cd ? { id: channelDoc.id, displayName: cd.displayName, username: cd.username || null, subscriberCount: cd.subscriberCount || 0, photoURL: cd.photoURL || null, tip: channelTip } : null });
}));
...
app.get('/channels/:uid', wrap(async (req, res) => {
  ...
  const d = doc.data();
  const { displayName, bio, photoURL, coverURL, username, subscriberCount, videoCount, createdAt } = d;
  // Tip details are exposed ONLY when the creator has explicitly enabled tips.
  const tip = (d.tipEnabled && d.tipNumberLocal && d.tipCountry) ? {
    enabled: true,
    number: d.tipNumberLocal,
    network: d.tipNetwork || null,
    country: d.tipCountry,
    currency: (TIP_COUNTRIES[d.tipCountry] || {}).currency || null,
  } : { enabled: false };
  res.json({ id: doc.id, displayName, bio, photoURL: photoURL || null, coverURL: coverURL || null, username: username || null, subscriberCount: subscriberCount || 0, videoCount: videoCount || 0, createdAt, tip });
}));
...
app.put('/users/profile', authMiddleware, writeLimiter, wrap(async (req, res) => {
  ...
  // ── Tips (direct peer-to-peer, no payment integration) ──
  if (req.body.tipCountry !== undefined) {
    if (req.body.tipCountry && !TIP_COUNTRIES[req.body.tipCountry]) {
      return res.status(400).json({ error: 'Unsupported country' });
    }
    update.tipCountry = req.body.tipCountry || null;
  }
  if (req.body.tipNetwork !== undefined) update.tipNetwork = sanitizeText(req.body.tipNetwork, 40) || null;
  if (req.body.tipNumber !== undefined) {
    const country = req.body.tipCountry || (await db.collection('users').doc(req.user.uid).get()).data()?.tipCountry;
    if (req.body.tipNumber) {
      const norm = normalizeRegionalPhone(req.body.tipNumber, country);
      if (!norm) return res.status(400).json({ error: 'Invalid mobile money number for the selected country' });
      update.tipNumber = norm.e164;
      update.tipNumberLocal = norm.local;
    } else {
      update.tipNumber = null;
      update.tipNumberLocal = null;
    }
  }
  if (req.body.tipEnabled !== undefined) update.tipEnabled = !!req.body.tipEnabled;
